...
function Graph(props) {
  ...
  const Y2axisVisible = Object.entries(seriesVisibility)
    .filter(([k, v]) => ["recvQbytes", "sendQbytes"].includes(k))
    .some(([k, v]) => v);
  const Y3axisVisible = Object.entries(seriesVisibility)
    .filter(([k, v]) =>
      [
        "uSockServers",
        "uSockConn",
  ...
  const // destructuring objects
    { rows: allrows, filterRows, useUTC, info } = props;
  ...
  const // make array into an array that just has the invoice amounts
    _cpu = rows.map((d) => {
    ...
    _recvQbytes = rows.map((d) => {
      let newR = {
        x: new Date(d.date),
        y: Math.log10(Number(d.TotalRecvQ_bytes) + 1),
      }; // +1 to allow representing 0 on a log scale
    ...
    _sendQbytes = rows.map((d) => {
      let newR = {
        x: new Date(d.date),
        y: Math.log10(Number(d.TotalSendQ_bytes) + 1),
      }; // +1 to allow representing 0 on a log scale
      return newR;
    }),
  ...
  const options = {
      ...
      tooltip: {
        formatter: function () {
          ...
          if (useUTC) {
            return (
              "<b>" + date + " UTC </b><br/>" + this.series.name + ": " + this.y
            );
          }
          ...
          const offsetString =
            sign + Highcharts.pad(hours, 2) + ":" + Highcharts.pad(minutes, 2);
          ...
          return (
            "<b>" +
            new Intl.DateTimeFormat("en-US", options).format(this.x) +
            " UTC" +
            offsetString +
            "</b><br/>" +
            this.series.name +
      ...
      title: {
        text: `LSAF Resource Usage v2 ${info}`,
      },
      series: [
        ...
        {
          name: "Transient",
          data: _transient.slice(slice_start, slice_length),
        },
        { name: "SASwork", data: _saswork.slice(slice_start, slice_length) },
        {
          name: "Workspace",
          data: _workspace.slice(slice_start, slice_length),
        },
        ...
        {
          name: "recvQbytes",
          data: _recvQbytes.slice(slice_start, slice_length),
          yAxis: 1,
          dashStyle: "Dot",
          visible: seriesVisibility["recvQbytes"],
        },
        {
          name: "sendQbytes",
          data: _sendQbytes.slice(slice_start, slice_length),
          yAxis: 1,
          dashStyle: "Dot",
          visible: seriesVisibility["sendQbytes"],
        },
        // uSockServers, uSockConn, tcpActConn, tcpInactConn, httpsActConn, httpsInactConn
        {
          name: "uSockServers",
          data: _usockservers.slice(slice_start, slice_length),
          yAxis: 2,
          dashStyle: "Dash",
          visible: seriesVisibility["uSockServers"],
        }, // Initially hidden
        {
          name: "uSockConn",
          data: _usockconn.slice(slice_start, slice_length),
          yAxis: 2,
          dashStyle: "Dash",
          visible: seriesVisibility["uSockConn"],
        }, // Initially hidden
        {
          name: "tcpActConn",
          data: _tcpactconn.slice(slice_start, slice_length),
          yAxis: 2,
          dashStyle: "Dash",
          visible: seriesVisibility["tcpActConn"],
        }, // Initially hidden
        {
          name: "tcpInactConn",
          data: _tcpinactconn.slice(slice_start, slice_length),
          yAxis: 2,
          dashStyle: "Dash",
          visible: seriesVisibility["tcpInactConn"],
        }, // Initially hidden
        {
          name: "httpsActConn",
          data: _httpsactconn.slice(slice_start, slice_length),
          yAxis: 2,
          dashStyle: "Dash",
          visible: seriesVisibility["httpsActConn"],
        }, // Initially hidden
        {
          name: "httpsInactConn",
          data: _httpsinactconn.slice(slice_start, slice_length),
          yAxis: 2,
          dashStyle: "Dash",
          visible: seriesVisibility["httpsInactConn"],
        }, // Initially hidden
      ],
      xAxis: {
        type: "datetime",
        ...
        labels: {
          ...
          formatter: function () {
            ...
            return (
              moment(new Date(this.value)).format("ddd, MMM DD HH:mm") + offset
            );
          },
        },
        minRange: 1800000,
